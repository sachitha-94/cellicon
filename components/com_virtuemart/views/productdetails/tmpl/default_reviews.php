	...
	for ($num = 0; $num <= $maxrating; $num++) {
		$stars[] = '
				<span title="' . (JText::_ ("COM_VIRTUEMART_RATING_TITLE") . $num . '/' . $maxrating) . '" class="vmicon ratingbox" style="display:inline-block;width:' . 24 * $maxrating . 'px;">
					<span class="stars-blue" style="width:' . (24 * $num) . 'px">
					</span>
				</span>';
	} ?>
			...
			function check_reviewform() {
				...
				jQuery(function($) {
					...
					$('.write-reviews .ratingbox').mousemove( function(e){
						...
						var reit = $(this).children();

						var dif = e.pageX-jQuery('.write-reviews .ratingbox').offset().left; // nbr of pixels
						difRatio = Math.floor(dif/boxWidth* steps )+1; //step

						reit.width(difRatio*starSize);
...
//]]>
			...
			if ($review_editable) {
				...
				<br/>
				<span class="count_comment"><?php echo JText::_ ('COM_VIRTUEMART_REVIEW_COUNT') ?>
